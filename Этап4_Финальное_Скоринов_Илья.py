            ...
            while True:
                ...
                if choice == '💠 Выйти в меню':
                    ...
                    if len(notes) == 0:
                        ...
                        delete_note(notes)
                        display_notes(notes)
                elif choice == '🌀 Загрузить заметки':
                    jnotes = load_notes_json()
                    notes.extend(jnotes)
                    display_notes(notes)
        ...
        if note_number > 0 and note_number <= len(notes):
            ...
            while True:
                ...
                if choice == '💠 Назад':
                    ...
                    notes[note_number-1]['Заголовок'] = title_init()
...
# Функции инициализации данных: Имя пользователя, описание заметки, заголовок заметки
# Создание имени пользователя
def user_init(): return input(f"\n{f.WHITE}👤 Введите своё имя >> ")
# Создание описания заметки
def content_init(): return input(f"\n{f.WHITE}📝 Введите описание заметки >> ")
# Создание заголовка заметки
def title_init(): return input(f'\n{f.WHITE}📌 Введите заголовок заметки >> ')


# Функция нормализации даты
def normalize_date(date): return date.replace(".", "-").replace("/", "-")
...
'''
Я не стал добавлять импорт заметок с файлов с расширениями
YAML и TXT, так как просто не вижу смысла импортировать
несколько буквально одинаковых заметок с разных файлов.
В целом функции импорта с различных файлов мне понятна
и одного JSON'а должно быть достаточно, так как по моему
мнению он самый удобный и гибкий
'''

# Экспорт заметок в YAML
def save_notes_yaml(notes):
    with open('ynotes.yaml', mode='w', encoding='utf-8') as file:
        t_file = yaml.dump(notes, file, indent=4, allow_unicode=True)


# Экспорт заметок в TXT
def save_notes_txt(notes):
    with open('notes.txt', mode='w', encoding='utf-8') as file:
        t_file = json.dump(notes, file, indent=4, ensure_ascii=False)
        # С помощью модуля JSON, так как с текстом потом легче работать


# Экспорт заметок в JSON
def save_notes_json(notes):
    with open('jnotes.json', mode='w', encoding='utf-8') as file:
        j_file = json.dump(notes, file, indent=4, ensure_ascii=False)
        # Данные экспортируются и перезаписываются при закрытии программы

# Импорт заметок из JSON
def load_notes_json():
    try:
        with open('jnotes.json', mode='r', encoding='utf-8') as file:
            info = json.load(file)
        return info
